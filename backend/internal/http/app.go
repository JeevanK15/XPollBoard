...
func (a *App) Register(r *gin.Engine) {
	...
	api.GET("/polls/:shareID/comments", a.getComments)
	api.POST("/polls/:shareID/comments", a.addComment)
	api.GET("/polls/:shareID/reactions", a.getReactions)
	api.POST("/polls/:shareID/reactions", a.addReaction)
	api.POST("/polls/:shareID/access", a.accessPoll)
	authed := api.Group("", a.auth())
	authed.POST("/polls", a.createPoll)
	authed.PUT("/polls/:shareID", a.updatePoll)
